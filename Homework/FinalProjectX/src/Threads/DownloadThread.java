 ...
 */
                ...
                /*
                Класс, отвечающий за загрузку файлов из интернета
                 */
...
public class DownloadThread extends PubData implements Runnable {
    ...
    public synchronized void run() {
        ...
        System.out.println("Поток "+threadDownloader.getName()+" запущен,идёт загрузка, один момент...");
            ...
            try {
                fileDownloader(PubData.JSON_URL);
                fileDownloader(PubData.XML_URL);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public String fileDownloader(String fileURL) throws IOException {
        ...
        try {
            URL urlJSON = new URL(PubData.JSON_URL);
            URL urlXML = new URL(PubData.XML_URL);
